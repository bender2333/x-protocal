...
#include "Platform/MBL/mbl_region.h"
...
#if defined MCUBOOT_SWAP_USING_SCRATCH && !defined GD32G5X3_BANKSWAP
...
int boot_write_image_ok_unset(const struct flash_area *fap);
int boot_write_copy_done_unset(const struct flash_area *fap);
int boot_write_magic_unset(const struct flash_area *fap);
...
boot_perform_update(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    if(((swap_type) == BOOT_SWAP_TYPE_REVERT) || ((swap_type) == BOOT_SWAP_TYPE_PERM)) {
        swap_set_image_ok(BOOT_CURR_IMG(state));
    }
#endif /* GD32F5XX_BANKS_SWP_USED */
...
#ifdef GD32F5XX_BANKS_SWP_USED
        flash_area_open(FLASH_AREA_0_ID, &fap);
        boot_write_magic(fap);

        flash_area_open(FLASH_AREA_1_ID, &fap);
        boot_write_image_ok_unset(fap);
        boot_write_copy_done_unset(fap);
        boot_write_magic_unset(fap);
#endif /* GD32F5XX_BANKS_SWP_USED */
        
#ifdef GD32G5X3_BANKSWAP
        flash_area_open(run_img, &fap);
        boot_write_magic(fap);
            
        flash_area_open(1 - run_img, &fap);
        boot_write_image_ok_unset(fap);
        boot_write_copy_done_unset(fap);
        boot_write_magic_unset(fap);
#endif
...
check_downgrade_prevention(struct boot_loader_state *state)
{
    ...
    if (rc < 0) {
        ...
        BOOT_LOG_INF("Primary version: %d.%d.%d, secondary version: %d.%d.%d", \
                    ...
                    (&boot_img_hdr(state, BOOT_SECONDARY_SLOT)->ih_ver)->iv_major, \
                    (&boot_img_hdr(state, BOOT_SECONDARY_SLOT)->ih_ver)->iv_minor, \
                    (&boot_img_hdr(state, BOOT_SECONDARY_SLOT)->ih_ver)->iv_revision);
   ...
   TARGET_STATIC boot_sector_t scratch_sectors[BOOT_MAX_IMG_SECTORS];
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        if (FIH_NOT_EQ(fih_rc, FIH_SUCCESS) ||
            ...
            BOOT_LOG_INF("validate primary solt succeed.");
        }
#else
