...
int
boot_write_image_ok_unset(const struct flash_area *fap)
{
    uint32_t off;
    uint8_t erased_val;
    ...
    off = boot_image_ok_off(fap);
    erased_val = flash_area_erased_val(fap);
    BOOT_LOG_DBG("writing image_ok unset; fa_id=%d off=0x%lx (0x%lx)",
                 flash_area_get_id(fap), (unsigned long)off,
                 (unsigned long)(flash_area_get_off(fap) + off));
    return boot_write_trailer_flag(fap, off, erased_val);
}

int
boot_write_copy_done_unset(const struct flash_area *fap)
{
    uint32_t off;
    uint8_t erased_val;
    ...
    erased_val = flash_area_erased_val(fap);
    BOOT_LOG_DBG("writing copy_done_unset; fa_id=%d off=0x%lx (0x%lx)",
                 flash_area_get_id(fap), (unsigned long)off,
                 (unsigned long)(flash_area_get_off(fap) + off));
    return boot_write_trailer_flag(fap, off, erased_val);
}
...
int
boot_write_magic_unset(const struct flash_area *fap)
{
    uint32_t off;
    uint32_t pad_off;
    int rc;
...
//    memcpy(&magic[BOOT_MAGIC_ALIGN_SIZE - BOOT_MAGIC_SZ], BOOT_IMG_MAGIC, BOOT_MAGIC_SZ);
    ...
    BOOT_LOG_DBG("writing magic unset; fa_id=%d off=0x%lx (0x%lx)",
                 flash_area_get_id(fap), (unsigned long)off,
                 (unsigned long)(flash_area_get_off(fap) + off));
    rc = flash_area_write(fap, pad_off, &magic[0], BOOT_MAGIC_ALIGN_SIZE);
