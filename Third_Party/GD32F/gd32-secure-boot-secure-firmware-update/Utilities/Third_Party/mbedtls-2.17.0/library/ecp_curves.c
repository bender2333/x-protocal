...
static mbedtls_mpi_uint Rp192k1[2];
static mbedtls_mpi_uint Rp224k1[2];
static mbedtls_mpi_uint Rp256k1[2];
static mbedtls_mpi_uint one[1];
void mbedtls_ecp_curve_val_init()
{
	one[0] = 1;

	Rp192k1[0] = BYTES_TO_T_UINT_4( 0xC9, 0x11, 0x00, 0x00);
	Rp192k1[1] = BYTES_TO_T_UINT_4( 0x01, 0x00, 0x00, 0x00);

	Rp224k1[0] = BYTES_TO_T_UINT_4( 0x93, 0x1A, 0x00, 0x00);
	Rp224k1[1] = BYTES_TO_T_UINT_4( 0x01, 0x00, 0x00, 0x00);

	Rp256k1[0] = BYTES_TO_T_UINT_4( 0xD1, 0x03, 0x00, 0x00);
	Rp256k1[1] = BYTES_TO_T_UINT_4( 0x01, 0x00, 0x00, 0x00);
}
#endif
/*
...
#ifdef REPLACE_AEABI_MEMCLR
#define LAST        p += WIDTH; *p = c; p += 1; memset(p, 0, (end - p) * sizeof( mbedtls_mpi_uint )); p = end;
#else
...
#ifdef REPLACE_AEABI_MEMCLR
    if (P224_WIDTH_MAX < M.n)
        memset(&Mp[P224_WIDTH_MAX], 0, (M.n - P224_WIDTH_MAX) * sizeof(mbedtls_mpi_uint));
#else
...
#else
    return( ecp_mod_koblitz( N, Rp192k1, 192 / 8 / sizeof( mbedtls_mpi_uint ), 0, 0, 0 ) );
#endif
}
#endif /* MBEDTLS_ECP_DP_SECP192K1_ENABLED */
...
#endif
#else //MBEDTLS_ROM_TEST
#if defined(MBEDTLS_HAVE_INT64)
    return( ecp_mod_koblitz( N, Rp224k1, 4, 1, 32, 0xFFFFFFFF ) );
#else
    return( ecp_mod_koblitz( N, Rp224k1, 224 / 8 / sizeof( mbedtls_mpi_uint ), 0, 0, 0 ) );
#endif
#endif
}
...
#else
    return( ecp_mod_koblitz( N, Rp256k1, 256 / 8 / sizeof( mbedtls_mpi_uint ), 0, 0, 0 ) );
#endif
}
#endif /* MBEDTLS_ECP_DP_SECP256K1_ENABLED */
