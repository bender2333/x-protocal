...
def parse_der(data):
    ...
    if tag == 0x02:  # Integer
        if contents[0] == 0 :
            contents = data[3+length_bytes : 3+length_bytes+length-1]
        return tag, total_length, contents
...
class Image():
    ...
    def create(self, key, public_key_format, enckey, dependencies=None,
        ...
        if key is not None:
            ...
            if key.sig_type() == "ECDSA256_SHA256" :
                public_key = key.key.public_key()
                public_key_bytes_x = public_key.public_numbers().x.to_bytes(32, byteorder='big')
                # print("public key x is: ", public_key_bytes_x.hex())
                public_key_bytes_y = public_key.public_numbers().y.to_bytes(32, byteorder='big')
                # print("public key y is: ", public_key_bytes_y.hex())
                sha = hash_algorithm()
                # sha.update(pub)
                sha.update(public_key_bytes_x)
                sha.update(public_key_bytes_y)
            elif key.sig_type() == "PKCS1_PSS_RSA2048_SHA256" :
                public_key = key.key.public_key()
                public_key_hex_n = public_key.public_numbers().n.to_bytes(256, byteorder='big')
                sha = hash_algorithm()
                sha.update(public_key_hex_n)
                print("pub key is:", public_key_hex_n.hex())
            else:
                sha = hash_algorithm()
                sha.update(pub)
            pubbytes = sha.digest()
            print("pub key hash is:", pubbytes.hex() )
        ...
        if key is not None or fixed_sig is not None:
            ...
            if key is not None and fixed_sig is None:
                ...
                if key.sig_type() == "ECDSA256_SHA256" :
                    # print("tlv sig:", sig.hex())
                    parsed_data = parse_der(sig)
                    # print("tlv sig:", parsed_data[2][0].hex() + parsed_data[2][1].hex())
                    tlv.add(key.sig_tlv(), parsed_data[2][0] + parsed_data[2][1])
                    self.signature = parsed_data[2][0] + parsed_data[2][1]
                elif key.sig_type() == "PKCS1_PSS_RSA2048_SHA256" :
                    print("tlv sig:", sig.hex())
