...
def gen_rsa2048(keyfile, passwd):
    ...
    private_key = keys.RSA.generate()
    private_key.export_private(path=keyfile, passwd=passwd)
    public_key = private_key.key.public_key()
    public_key_hex_n = public_key.public_numbers().n.to_bytes(256, byteorder='big').hex()

    with open("mcuboot_rsa2048_public_key.txt", "w") as f:
        f.write(public_key_hex_n)
        f.close()

    with open("mcuboot_rsa2048_public_key.bin", "wb") as f:
        f.write(bytes.fromhex(public_key_hex_n))
        f.close()
...
def sign(key, public_key_format, align, version, pad_sig, header_size,
    ...
    img.load(infile)
    key = load_key(key) if key else None
    print("sig type is: ", key.sig_type())
    # print(f"{__file__} : {sys._getframe().f_lineno}")
    # raw_private_key = key.key.private_numbers().private_value.to_bytes(32, byteorder='big')
    # hex_private_key = raw_private_key.hex()
    # print("private key is: ", hex_private_key)
