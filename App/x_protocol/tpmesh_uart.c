...
static int tpmesh_uart6_init_internal(void) {
  ...
  gpio_output_options_set(UART6_GPIO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ,
                          UART6_TX_PIN);
...
int tpmesh_uart6_init(void) { return tpmesh_uart6_init_internal(); }
...
int tpmesh_uart6_send(const uint8_t *data, uint16_t len) {
  ...
  for (uint16_t i = 0; i < len; i++) {
    ...
    while ((RESET == usart_flag_get(UART6_PERIPH, USART_FLAG_TBE)) &&
           (--timeout > 0))
      ;
    if (timeout == 0) {
      return -1;
  ...
  /* 等待最后一字节发送完成 */
  uint32_t timeout = TPMESH_UART6_TX_TIMEOUT;
  while ((RESET == usart_flag_get(UART6_PERIPH, USART_FLAG_TC)) &&
         (--timeout > 0))
    ;
