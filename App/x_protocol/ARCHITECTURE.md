...
为避免歧义，统一按“两层格式”理解一条 Mesh 报文：

1. AT 外层（命令/URC 头）
2. `HEX_DATA` 内层（Mesh 隧道帧）
...
AT+SEND 外层（发送）
┌────────┬────────┬──────────────┬────────┐
│ DEST   │  LEN   │   HEX_DATA   │  TYPE  │
│ 2B地址 │ 字节数 │ LEN 字节负载 │  1B    │
└────────┴────────┴──────────────┴────────┘
AT+SEND=<DEST>,<LEN>,<HEX_DATA>,<TYPE>

+NNMI 外层（接收，优先格式）
┌────────┬────────┬────────┬────────┬──────────────┐
│  SRC   │  DEST  │  RSSI  │  LEN   │   HEX_DATA   │
│ 2B地址 │ 2B地址 │ 1字段  │ 字节数 │ LEN 字节负载 │
└────────┴────────┴────────┴────────┴──────────────┘
+NNMI:<SRC>,<DEST>,<RSSI>,<LEN>,<HEX_DATA>

HEX_DATA 内层（二进制隧道帧）
┌────────┬──────────┬─────────┬────────────────────┐
│ L2_HDR │ FRAG_HDR │ RULE_ID │    RULE_PAYLOAD    │
│   1B   │    1B    │   1B    │    0..(MTU-3)B     │
└────────┴──────────┴─────────┴────────────────────┘
```
...
- `SRC` / `DEST`：Mesh 地址（16-bit，文本中为 4 位十六进制）
- `LEN`：`HEX_DATA` 的字节数，不是十六进制字符数
- `TYPE`：AT+SEND 模块参数（当前业务固定传 `0`）
- `L2_HDR`：bit7=广播标记，bit[6:0] 保留
- `FRAG_HDR`：bit7=last，bit[6:0]=seq

长度规则：

- `strlen(HEX_DATA) == LEN * 2`
- `LEN <= TPMESH_MTU`（当前 `TPMESH_MTU=200`）
...
### 6.0.2 子类型码总表（`RULE_ID=0x10`）

| 子类型码 | 名称 | 方向 | 说明 |
|---|---|---|---|
| `0x01` | `REGISTER` | Edge -> Top | 注册请求 |
| `0x02` | `REGISTER_ACK` | Top -> Edge | 注册确认 |
| `0x03` | `HEARTBEAT` | Edge -> Top | 心跳请求 |
| `0x04` | `HEARTBEAT_ACK` | Top -> Edge | 心跳响应 |
